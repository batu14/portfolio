...
const Index = () => {
  ...
  const skills = [
    { name: "REACT", level: 5, icon: CiLaptop, description: "React.js, Tailwind CSS ve GSAP gibi modern teknolojileri kullanarak kullanıcılarla etkileşimde kusursuz deneyimler oluşturuyorum." },
    { name: "Tailwind", level: 4, icon: CiLaptop, description: "Tailwind CSS, GSAP ve modern teknolojileri kullanarak kullanıcılarla etkileşimde kusursuz deneyimler oluşturuyorum." },
    { name: "PHP", level: 4, icon: CiLaptop, description: "PHP, Node.js ve MySQL gibi güçlü altyapılarla sağlam ve güvenilir sistemler geliştiriyorum." },
    { name: "MYSQL", level: 4, icon: CiLaptop, description: "MySQL, PostgreSQL ve MongoDB gibi güçlü altyapılarla sağlam ve güvenilir sistemler geliştiriyorum." },
    { name: "REACT", level: 5, icon: CiLaptop, description: "React.js, Tailwind CSS ve GSAP gibi modern teknolojileri kullanarak kullanıcılarla etkileşimde kusursuz deneyimler oluşturuyorum." },
    { name: "Tailwind", level: 4, icon: CiLaptop, description: "Tailwind CSS, GSAP ve modern teknolojileri kullanarak kullanıcılarla etkileşimde kusursuz deneyimler oluşturuyorum." },
    { name: "PHP", level: 4, icon: CiLaptop, description: "PHP, Node.js ve MySQL gibi güçlü altyapılarla sağlam ve güvenilir sistemler geliştiriyorum." },
    { name: "MYSQL", level: 4, icon: CiLaptop, description: "MySQL, PostgreSQL ve MongoDB gibi güçlü altyapılarla sağlam ve güvenilir sistemler geliştiriyorum." },
  ...
  useGSAP(() => {
    ...
    mm.add("(min-width: 768px)", () => {
      // Hero image fade in
      gsap.fromTo(".hero-image", { opacity: 0 }, { opacity: 1, delay: 2, duration: 1 });

      // Down arrow animation
      gsap.fromTo(".down", { opacity: 0, y: -30 }, { opacity: 1, y: 0, duration: 1, ease: "power2.out", delay: 3 });
      
      // Scroll-triggered image animations (masaüstü için daha karmaşık)
      gsap.utils.toArray(".scroll-image").forEach((item, index) => {
        const isEven = index % 2 === 0;
        gsap.fromTo(item,
          { rotationZ: isEven ? 60 : -60, x: isEven ? -400 : 400, opacity: 0, y: 150, scale: 0.6, filter: "blur(10px)" },
          { rotationZ: 0, x: 0, opacity: 1, y: 0, scale: 1, filter: "blur(0px)", duration: 1.5, ease: "power3.out",
            scrollTrigger: {
              trigger: item,
              start: "top 80%",
              end: "top 50%",
    ...
    gsap.utils.toArray(".social-link").forEach((link, i) => {
        gsap.fromTo(link,
            { opacity: 0, x: -20 },
            {
                opacity: 1, x: 0, duration: 0.5, delay: 0.6 + i * 0.1, ease: "power2.out",
                scrollTrigger: {
                    trigger: contactSectionRef.current,
                    start: "top 70%",
                    toggleActions: "play none none reverse",
  ...
  }, { scope: mainContainer }); // Ana container'ı scope olarak bağlıyoruz.
    ...
    const interval = setInterval(() => {
      ...
      if (svgContainer.current) {
        gsap.to(svgContainer.current, {
          opacity: 0, scale: 0.9, rotation: -5, filter: "blur(2px)", duration: 1, ease: "power3.inOut",
          onComplete: () => {
            setCurrentIconIndex((prevIndex) => (prevIndex + 1) % Icons.length);
            
            gsap.fromTo(svgContainer.current,
              { opacity: 0, scale: 1.1, rotation: 5, filter: "blur(2px)" },
              { opacity: 1, scale: 1, rotation: 0, filter: "blur(0px)", duration: 1, ease: "power3.out" }
            );
          }
        });
      }
  ...
  return (
    <div ref={mainContainer} className="w-full flex items-start justify-start h-auto flex-col bg-white">
      {/* Hero Section */}
      <div className="w-full relative p-4 gap-4 px-12 flex items-start justify-center min-h-screen">
        <div className="w-full md:w-1/2 flex items-center gap-8 min-h-screen justify-center flex-col">
          <div className="space-y-6">
            <h1 className="text-6xl tracking-tight font-light text-gray-900 text">Batuhan ÇİFTÇİ</h1>
            <h2 className="text-3xl font-light text-gray-600 text">Full Stack Developer</h2>
            <p className="text-lg leading-relaxed text-gray-500 text max-w-lg">
              Kullanıcı deneyimini merkeze alan, modern ve etkili web çözümleri geliştiren tutkulu bir yazılım geliştiricisi.
            </p>
          </div>
        </div>
      ...
      <section className="w-full p-12 min-h-screen h-auto relative grid grid-cols-1 md:grid-cols-2 gap-4 place-items-center bg-gray-50">
        {images.map((item, index) => (
          <div key={index} className="bg-white aspect-video scroll-image flex items-center justify-center w-full h-full rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow duration-300">
            <span className="text-gray-400 text-lg font-light">Project {item + 1}</span>
          </div>
        ))}
      </section>

      {/* Skills Section - Refactored with pin:true */}
      <section ref={skillsContainer} className="w-full py-16 content-box flex flex-col-reverse md:flex-row items-start justify-start gap-8 bg-white">
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              <p className="text-gray-600 line leading-relaxed">
                Merhaba! Ben Batuhan, kullanıcı deneyimini merkeze alan bir Full Stack Web Developer'ım. Yazılım dünyasına olan ilgim, yalnızca bir meslekten öte; bir tutku, bir yaşam biçimi.
              </p>
              <p className="text-gray-600 line leading-relaxed">
                Frontend tarafında React.js, Tailwind CSS ve GSAP gibi modern teknolojileri kullanarak kullanıcılarla etkileşimde kusursuz deneyimler oluşturuyorum. Backend tarafında ise PHP, Node.js ve MySQL gibi güçlü altyapılarla sağlam ve güvenilir sistemler geliştiriyorum.
              </p>
              <p className="text-gray-600 line leading-relaxed">
                Geliştirdiğim projelerde her zaman şuna dikkat ederim: Bir kullanıcı, sayfaya geldiğinde nereye bakmalı, ne hissetmeli ve ne kadar kolay etkileşim kurmalı? Bu yüzden kullanıcı arayüzü tasarımında sadece güzel görünmesi değil, kullanılabilirliği yüksek çözümler üretmeyi önceliklendiriyorum.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* Contact Section */}
      <section ref={contactSectionRef} className="min-h-screen w-full bg-gradient-to-br from-gray-50 via-white to-gray-100 flex flex-col lg:flex-row items-center justify-center relative overflow-hidden p-4 md:p-8 lg:p-12">
        <div className="hidden lg:flex w-full lg:w-1/2 h-1/2 lg:h-full items-center justify-center relative z-10 p-4 lg:p-0">
          <div ref={svgContainer} className="w-64 h-64 flex items-center justify-center relative group">
            {Icons[currentIconIndex] && (
              <>
                ...
                <div className="relative z-10 drop-shadow-lg">{Icons[currentIconIndex].svg}</div>
              </>
            )}
          </div>
        </div>
        <div className="w-full lg:w-1/2 flex items-center justify-center p-0 md:p-4 lg:p-12 contact-content">
          <div className="w-full space-y-6 md:space-y-8 bg-white p-4 md:p-6 lg:p-8 rounded-lg shadow-xl border border-gray-100">
            <div className="space-y-2 md:space-y-3">
              <h2 className="text-2xl md:text-3xl font-light text-gray-800">İletişime Geçin</h2>
            ...
            <div className="space-y-3 md:space-y-4">
              <input type="text" className="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-300 transition-all duration-200 placeholder-gray-400" placeholder="Ad Soyad" />
              <input type="email" className="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-300 transition-all duration-200 placeholder-gray-400" placeholder="Email" />
              <textarea rows={4} className="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-200 rounded-md focus:outline-none focus:ring-1 focus:ring-gray-300 transition-all duration-200 resize-none placeholder-gray-400" placeholder="Mesajınız..."/>
